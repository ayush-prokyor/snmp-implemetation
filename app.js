...
app.post('/api/snmp/set', (req, res) => {
  ...
  if (!oid || !type || value === undefined) {
    return res.status(400).json({ error: 'Missing required parameters: oid, type, value' });
  }
...
app.get('/api/snmp/getbulk', (req, res) => {
  ...
  session.getBulk([oid], nonRepeaters, maxRepetitions, (error, varbinds) => {
    ...
    if (!varbinds || varbinds.length === 0) {
      return res.json({ results: [], message: 'No data returned from SNMP agent' });
    }
...
app.post('/api/polling/config', (req, res) => {
  const { isEnabled, interval, type, oids, oid, nonRepeaters, maxRepetitions } = req.body;
  ...
  if (maxRepetitions !== undefined) pollingConfig.maxRepetitions = maxRepetitions;
...
function pollGetBulk() {
  ...
  session.getBulk([pollingConfig.oid], pollingConfig.nonRepeaters, pollingConfig.maxRepetitions, (error, varbinds) => {
    if (error) {
...
function startPolling() {
  ...
  console.log(`Polling started: ${pollingConfig.type.toUpperCase()} every ${pollingConfig.interval}ms`);
}
